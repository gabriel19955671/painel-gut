...
# INSTRUÇÕES
with st.expander("📌 Instruções de Uso do Diagnóstico", expanded=True):
    instrucoes = st.text_area("Digite as instruções finais para o cliente (você pode expandir essa área):", height=300)
...
with col1:
    if st.button("📄 Gerar Apenas Gráfico Radar"):
        fig_radar = go.Figure()
        ...
        with open("Radar_Separado.pdf", "rb") as f:
            st.download_button('📥 Baixar Radar PDF', f, file_name="Radar_Separado.pdf", mime="application/pdf")

with col2:
    if st.button("📄 Gerar Apenas Matriz GUT"):
        pdf = FPDF()
        pdf.add_page()
        pdf.set_font("Arial", 'B', 16)
        ...
        with open("GUT_Separado.pdf", "rb") as f:
            st.download_button('📥 Baixar GUT PDF', f, file_name="GUT_Separado.pdf", mime="application/pdf")

# PDF COMPLETO
if st.button("📄 Gerar PDF Diagnóstico Completo"):
    fig_radar = go.Figure()
    if not df_radar.empty:
        df_agrupado = df_radar.groupby('Área')['Avaliação'].mean().reset_index()
        df_full = pd.DataFrame({'Área': df_radar['Área'].unique()})
    ...
    with open("Diagnostico_360_Resumo.pdf", "rb") as f:
        st.download_button('📥 Baixar PDF Completo', f, file_name="Diagnostico_360_Resumo.pdf", mime="application/pdf")
