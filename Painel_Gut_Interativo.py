...
with aba4:
    ...
    opcoes_exportacao = st.selectbox("Escolha o conteúdo para exportar:", ["PDF Completo", "Gráfico Radar", "Matriz GUT", "Plano de Ação", "Instruções"])
    buf_radar = BytesIO()
    fig_radar.write_image(buf_radar, format='png')

    if st.button("Gerar PDF", key="exportar_pdf"):
        pdf = FPDF()
        pdf.set_auto_page_break(auto=True, margin=15)

        if opcoes_exportacao == "Gráfico Radar":
        ...
        elif opcoes_exportacao == "Instruções":
            pdf.add_page()
            pdf.set_font("Arial", 'B', 16)
